...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform.translate[2][0] += 0.001f;

        //2. Rotation 을 프레임당 1도씩 누적시켜서 물체를 회전시켜보세요.
        turn += 1.0;

        transform.rotation = glm::mat3(
            glm::cos(glm::radians(turn)), -glm::sin(glm::radians(turn)), 0,
            glm::sin(glm::radians(turn)), glm::cos(glm::radians(turn)), 0,
            0, 0, 1
        );
        // cosθ -sinθ 0
        // sinθ cosθ 0
        //  0     0    1
        //이 행령은 회전행령으로
        //turn이라는 float함수를 만들어 각 회전의 원점이 되는 cos,sin에다가 값을 넣어 호도값을 증가시켰다.    
       ...
       //   (1.3배 이상이 되면 줄어들고 0.7배 이하가 되면 다시 늘어나게 만드시오)
        ...
        {
            bigger = false;
            transform.scale[0][0] = 1.3f;
            transform.scale[1][1] = 1.3f;

        }
        else if (transform.scale[0][0] <= 0.7f)
        {
            bigger = true;
            transform.scale[0][0] = 0.7f;
            transform.scale[1][1] = 0.7f;

        }
        //scale의 행령은 다른 행령보다 간단하게 x,y의 값을 얼마나 키울 것인지에 따라 그냥 곱하거나 값을 계속 더하면 된다.
        //EX)두배를 키우고 싶다. (2,1) -> (4,2) 
        //W 0 0
        //0 H 0
        //0 0 1 
        //이 행렬 부분에 W,H의 값을 1.3까지 계속 늘릴려면 (0.01+=W)(0.01+=H)식 으로 하여 1.3의 값이 될때까지 반복한다. 
        //반대로 1.3의 값이 되어 0.7까지 계속 줄일려면 W,H의 값을 (0.01-=W)(0.01-=H)식으로 하여 0.7의 값이 될때까지 반복한다.
        //////////////////////////////////////////////////////////////////////////////////////////
